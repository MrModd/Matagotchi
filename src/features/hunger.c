...
static void check(const struct ApplicationContext *context, uint32_t current_timestamp, struct GameEvents *game_events) {
...
static bool apply(struct ApplicationContext *context, struct GameEvents game_events) {
		struct GameState *game_state = context->game_state;
